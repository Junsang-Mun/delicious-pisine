...
<label for="{id}" class="card bg-base-100 shadow m-5 w-80 xl:w-96" style="cursor: pointer;">
	<div class="card-body">
...
<label for="{id}" class="modal modal-bottom sm:modal-middle">
	<div class="modal-box">
		...
		<flex class="p-2">
			...
			{#if credit != undefined}
			<button class="badge badge-primary" on:click={showCreditUser(credit)}>{credit}</button>
			{/if}
		</flex>
		<p class="p-2 pt-4">{@html detail}</p>
		<div class="modal-action">
			{#if lat != undefined && lng != undefined}
				<button class="btn" on:click={() => {provideDirections(lat, lng, 'N', name)}}>네이버맵 길찾기</button>
			{/if}
