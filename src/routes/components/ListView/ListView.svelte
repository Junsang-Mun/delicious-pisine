      ...
      {#each $restaurantDatas as rd}
        {#if rd.coordinate.loc == location}
          <Card
            ...
            detail={rd.detail}
            credit={rd.credit}
            lat={rd.coordinate.lat}
            lng={rd.coordinate.lng}
            loc={rd.coordinate.loc}
          />
        {/if}
      {:else}
        <div class="grid place-items-center p-10">
          <button
            disabled
            type="button"
            class="text-white bg-primary focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 dark:bg-primary inline-flex items-center"
          >
            <svg
              role="status"
