                    ...
                    <div class="card-actions justify-end">
                        ...
                        <label for="rd-modal-{JSON.stringify(rd.id)}" class="modal cursor-pointer">
                            <label class="modal-box relative" for="">
                                ...
                                <button class="btn btn-outline" on:click="{() => giveMeWay((JSON.stringify(rd.attributes.Lng).replace(/\"/g, ''), JSON.stringify(rd.attributes.Lat).replace(/\"/g, '')), JSON.stringify(rd.attributes.Name).replace(/\"/g, ''), 'N')}">네이버 지도 길찾기</button>
